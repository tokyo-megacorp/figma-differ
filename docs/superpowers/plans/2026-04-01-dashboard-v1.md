# Fig-Diff Dashboard v1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Generate a single self-contained `review.html` from `review.json` + `index.json` + Figma image URLs that lets developers see exactly what changed between two design snapshots.

**Architecture:** Node.js generator (`render-review.js`) reads JSON data files, optionally fetches image URLs from Figma API, and injects everything into an HTML template. The template uses vanilla JS for three views: Index (pick a diff), Accordion (scan changes), Detail (inspect a frame). System-adaptive theme, JetBrains Mono for diffs.

**Tech Stack:** Node.js (already required for structural-diff.js), vanilla HTML/CSS/JS, Google Fonts CDN (JetBrains Mono), Figma REST API (image URLs only)
...
```
scripts/
  render-review.js              # NEW — Node.js generator, reads data, outputs review.html
  figma-api.sh                  # MODIFY — add fetch_image_urls command (URLs only, no download)
  compile-review.sh             # EXISTING — no changes
  structural-diff.js            # EXISTING — no changes
```

The HTML template is embedded as a template literal inside `render-review.js` rather than a separate file. This keeps the generator self-contained — one file to maintain, no path resolution issues.

---

### Task 1: Add `fetch_image_urls` to figma-api.sh

Adds a new command that returns a JSON map of `{nodeId: s3Url}` without downloading any PNGs. The existing `fetch_batch_images` downloads files — this variant just returns URLs for embedding in HTML.

**Files:**
- Modify: `scripts/figma-api.sh:252-266` (dispatch section + new function)

- [ ] **Step 1: Write the `fetch_image_urls` function**

Add before the dispatch section in `scripts/figma-api.sh`:
...
```bash
# Test with a small set of known node IDs from the existing index
bash scripts/figma-api.sh fetch_image_urls 5nIxJq1CzXIipSFfjs8eMQ "1:683,1431:33250" | jq .
```

Expected: JSON object with node IDs as keys and S3 URLs as values.

- [ ] **Step 4: Commit**

```bash
git add scripts/figma-api.sh
git commit -m "feat(api): add fetch_image_urls command for URL-only batch image fetching"
```

---

### Task 2: Build `render-review.js` — data loading and CLI

The core generator script. This task handles argument parsing, data loading (review.json, index.json, structural diffs), and optionally fetching image URLs. No HTML generation yet — just the data pipeline.

**Files:**
- Create: `scripts/render-review.js`

- [ ] **Step 1: Write the data loading module**
...
```bash
chmod +x scripts/render-review.js
node scripts/render-review.js --help 2>&1 || true
```

Expected: prints usage line and exits.

- [ ] **Step 3: Commit**

```bash
git add scripts/render-review.js
git commit -m "feat(dashboard): add render-review.js data pipeline and CLI"
```

---

### Task 3: Build the HTML template — CSS and theme system

The CSS foundation: system-adaptive theme, layout structure, typography, severity colors, diff hunk styling. This is the biggest single task. The HTML template is a template literal inside `render-review.js`.

**Files:**
- Modify: `scripts/render-review.js` (replace `generateHtml` placeholder)

- [ ] **Step 1: Replace the `generateHtml` function with the full template**

Replace the placeholder `generateHtml` function in `scripts/render-review.js` with:
...
The `generateAppJs()` function doesn't exist yet (Task 4). For now, add a temporary stub after `generateHtml`:
...
Run:
```bash
node scripts/render-review.js 5nIxJq1CzXIipSFfjs8eMQ --no-images --no-open --output /tmp/test-review.html
open /tmp/test-review.html
```

Expected: Opens browser with JSON dump styled on dark background.

- [ ] **Step 3: Commit**

```bash
git add scripts/render-review.js
git commit -m "feat(dashboard): add HTML template with full CSS theme system"
```

---

### Task 4: Build the JavaScript app — Index screen

The vanilla JS that powers the Index screen. Reads `DATA.reviews` and renders diff range cards.

**Files:**
- Modify: `scripts/render-review.js` (replace `generateAppJs` stub)

- [ ] **Step 1: Write the app JS scaffold and Index screen renderer**

Replace the `generateAppJs` stub with:
...
Expected: Dark-themed page showing "fig-diff Profile, 548 frames tracked" with diff range cards (or "No diffs found" if no diffs exist yet).

- [ ] **Step 3: Commit**

```bash
git add scripts/render-review.js
git commit -m "feat(dashboard): implement Index screen with diff range cards"
```

---

### Task 5: Build the JavaScript app — Accordion screen

The main review view. Groups frames by page, collapsible cards with diff hunks, severity filter pills, keyboard navigation.

**Files:**
- Modify: `scripts/render-review.js` (replace `renderAccordion` placeholder)

- [ ] **Step 1: Replace the `renderAccordion` placeholder**

Replace the `renderAccordion` function inside `generateAppJs`:
...
- [ ] **Step 2: Test the Accordion screen**

This requires at least one `review.json` to exist. If none exist yet, create a test fixture:

```bash
mkdir -p ~/.figma-differ/5nIxJq1CzXIipSFfjs8eMQ/diffs/20260401T090000Z-vs-20260401T164047Z
# (Manually create a minimal review.json for testing, or run the actual diff pipeline)
```
...
Expected: Click a diff range → see accordion view with page groups, frame cards, filter pills.

- [ ] **Step 3: Commit**

```bash
git add scripts/render-review.js
git commit -m "feat(dashboard): implement Accordion screen with filters and diff hunks"
```

---

### Task 6: Build the JavaScript app — Detail screen

The deep-inspection view. Sidebar tree + full diff panel with change groups.

**Files:**
- Modify: `scripts/render-review.js` (replace `renderDetail` placeholder)

- [ ] **Step 1: Replace the `renderDetail` placeholder**

Replace the `renderDetail` function inside `generateAppJs`:
...
Expected: Click diff → accordion → click frame name → sidebar + detail panel with change groups.

- [ ] **Step 3: Commit**

```bash
git add scripts/render-review.js
git commit -m "feat(dashboard): implement Detail screen with sidebar and change groups"
```

---

### Task 7: Add lazy image loading and keyboard navigation

Wire up `IntersectionObserver` for lazy-loaded PNGs and keyboard shortcuts (`j`/`k`/`Enter`/`o`/`Esc`).

**Files:**
- Modify: `scripts/render-review.js` (add to end of `generateAppJs` return string)

- [ ] **Step 1: Add the lazy image observer and keyboard handler**

Append to the end of the `generateAppJs` return string (before the final backtick):
...
- [ ] **Step 2: Test keyboard navigation and lazy images**

```bash
node scripts/render-review.js 5nIxJq1CzXIipSFfjs8eMQ --no-open --output /tmp/test-review.html
open /tmp/test-review.html
```

Expected:
- In accordion: `j`/`k` moves blue focus ring between frame cards, `Enter` expands, `o` opens detail, `Esc` goes back to index
- In detail: `j`/`k` navigates sidebar, `Esc` goes back to accordion
- Images show shimmer placeholder, then load when scrolled into view (if `--no-images` was not used)

- [ ] **Step 3: Commit**

```bash
git add scripts/render-review.js
git commit -m "feat(dashboard): add lazy image loading and keyboard navigation"
```
...
Create a test fixture and verify the full pipeline works. Then test with actual Figma data if available.
...
```bash
mkdir -p ~/.figma-differ/TEST_KEY/diffs/20260401T090000Z-vs-20260401T164047Z
```

Create `~/.figma-differ/TEST_KEY/index.json`:
...
Create `~/.figma-differ/TEST_KEY/diffs/20260401T090000Z-vs-20260401T164047Z/review.json`:
...
```json
{
  ...
  "summary": {
    "total": 3, "structural": 1, "cosmetic": 1, "unchanged": 1,
    "approved": 0, "flagged": 0, "pending": 2
  },
  "byPage": [
    { "page": "Auth", "total": 2, "structural": 1, "cosmetic": 1 },
    { "page": "Main", "total": 1, "structural": 0, "cosmetic": 0 }
  ],
  "decisions": [
    {
      "nodeId": "1:1", "nodeName": "Login", "page": "Auth",
      "severity": "structural",
      "summary": "+2 nodes, 1 bbox change(s)",
      "nodeCountBefore": 15, "nodeCountAfter": 17, "nodeCountDelta": 2,
      "beforePath": "", "afterPath": "",
      "diffPath": "",
      "decision": "pending", "note": ""
    },
    {
      "nodeId": "1:2", "nodeName": "Signup", "page": "Auth",
      "severity": "cosmetic",
      "summary": "1 text change(s), 1 fill change(s)",
      "nodeCountBefore": 20, "nodeCountAfter": 20, "nodeCountDelta": 0,
      "beforePath": "", "afterPath": "",
      "diffPath": "",
      "decision": "pending", "note": ""
    },
    {
      "nodeId": "2:1", "nodeName": "Dashboard", "page": "Main",
      "severity": "unchanged",
      "summary": "no changes",
      "nodeCountBefore": 50, "nodeCountAfter": 50, "nodeCountDelta": 0,
      "beforePath": "", "afterPath": "",
      "diffPath": "",
      "decision": "pending", "note": ""
    }
...
- [ ] **Step 2: Generate and verify the review.html**

```bash
node scripts/render-review.js TEST_KEY --no-images --output /tmp/test-review.html
open /tmp/test-review.html
```

Verify:
- Index shows "Test File, 3 frames tracked" with one diff range card
- Click card → accordion shows Auth (2 changes) and Main (1 change, hidden by default filter)
- Toggle "unchanged" filter pill → Dashboard appears
- Expand Login → shows "+2 nodes, 1 bbox change(s)"
- Click "Login" name → detail view with sidebar
- `j`/`k` navigates, `Esc` goes back
- Light/dark theme switches with system preference

- [ ] **Step 3: Clean up test fixture**

```bash
rm -rf ~/.figma-differ/TEST_KEY
```

- [ ] **Step 4: Test with real data (if diffs exist)**

```bash
node scripts/render-review.js 5nIxJq1CzXIipSFfjs8eMQ --no-images --output /tmp/real-review.html
open /tmp/real-review.html
```
...
```bash
git add scripts/render-review.js
git commit -m "feat(dashboard): complete v1 — index, accordion, detail with lazy images"
```
...
| Task | What | Files |
|------|------|-------|
| 1 | `fetch_image_urls` in figma-api.sh | Modify: `scripts/figma-api.sh` |
| 2 | Data loading + CLI | Create: `scripts/render-review.js` |
| 3 | HTML template + CSS theme | Modify: `scripts/render-review.js` |
| 4 | Index screen JS | Modify: `scripts/render-review.js` |
| 5 | Accordion screen JS | Modify: `scripts/render-review.js` |
| 6 | Detail screen JS | Modify: `scripts/render-review.js` |
| 7 | Lazy images + keyboard nav | Modify: `scripts/render-review.js` |
| 8 | End-to-end test | No new files |
